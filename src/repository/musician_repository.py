...
class MusicianRepository:
    ...
    def get_musicians_by_names(self, musician_names) -> List[Musician]:
        musicians_df: pd.DataFrame = self.__postgres_client.retrieve_musicians(
            musician_names
        )
        musician_list: List[Musician] = list(
            musicians_df.apply(Mapper.convert_dataframe_to_musician, axis=1)
        )

        return musician_list

    def save(self, musician: Musician):
        musician_df: pd.DataFrame = Mapper.convert_dataframe_to_musician(musician)
