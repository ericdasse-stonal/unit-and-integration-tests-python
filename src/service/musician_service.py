...
class MusicianService:
    ...
    def get_all_musicians(self) -> List[Musician]:
        musician_names: List[
            str
        ] = self.__external_musician_client.get_all_musicians_names()
        musician_list: List[
            Musician
        ] = self.__musician_repository.get_musicians_by_names(musician_names)
        return musician_list

    def save(self, name: str, surname: str, age: int, instrument: str):
        musician: Musician = Musician(
            name=name, surname=surname, age=age, instrument=instrument
        )
